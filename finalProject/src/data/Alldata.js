// product data on home page
export const productData = [
  {
    id: 1,
    category:"Computers and Laptops",
    name: "Luxurious Purple Velvet Armchair in an Elegant Modern Setting",
    description:
      "Chair Style:- This is a luxurious armchair upholstered in rich purple velvet, featuring a tufted backrest. The chair has a rounded, elegant shape, and the legs appear to be metallic, possibly gold or brass. Setting:- The chair is placed in a sophisticated room with dark purple walls and gold accents. There's a modern floor lamp with a round globe-like shade on the left side, and the overall scene is well-lit with soft, ambient lighting. Design Aesthetic:- The aesthetic of the room is very high-end, with a focus on luxury and modern elegance. The deep purple tones paired with gold accents give it a regal, classy vibe.",
    image_Url: [
      {
        public_id: "test",
        url: 'images/img23.jpg',
      },
      {
        public_id: "test",
        url: 'images/img23.jpg',
      },
    ],
    shop: {
      name: "Luxurious Purple Velvet Armchair",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    price: 1099,
    discount_price: 1049,
    rating: 4,
    total_sell: 35,
    stock: 10,
  },
  {
    id: 2,
    category:"Mobile and Tablets",
    name: "Modern Blush Pink L-Shaped Sectional Sofa with Minimalist Design",
    description:
      "Sectional Sofa:- The room features a large, L-shaped sectional sofa in a blush pink or soft peach color. The sofa is low-profile, with clean lines and plush cushions, offering both comfort and style.Matching Ottoman :- In front of the sofa, there's a matching ottoman in the same blush tone, adding versatility and additional seating or leg rest.Coffee Table :- A minimalistic, rectangular wooden coffee table sits at the center of the seating area. Its natural wood finish complements the soft tones of the room while adding warmth and contrast.",
    image_Url: [
      {
        public_id: "test",
        url: 'images/img10.jpg',
      },
      {
        public_id: "test",
         url: 'images/img10.jpg',
      },
    ],
    shop: {
      name: "L-Shaped Sectional Sofa ",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    discount_price: 1099,
    rating: 5,
    total_sell: 80,
    stock: 10,
    category:"Mobile & Tablets"
  },
  {
    id: 3,
    category:"Computers and Laptop",
    name: "Cozy Oversized Cuddle Chair in a Relaxing Neutral Setting",
    description:
      "Chair Style:- This is a cozy, oversized round armchair, often referred to as a  snuggle chair  or cuddle chair. It has soft, textured upholstery with a checkered or tweed-like fabric in neutral tones. The chair is piled with multiple pillows, adding to its comfort and inviting aesthetic. Setting:- The chair is placed in a calm, neutral space, possibly a living room or a reading nook. The decor includes woven baskets, textured vases, and a soft, natural-looking plant arrangement, contributing to a warm, earthy feel.Design Aesthetic:- The space emphasizes comfort and relaxation, with natural materials and a subdued color palette, perfect for a cozy corner in a modern or contemporary home",
    image_Url: [
      {
        public_id: "test",
        url: 'images/img22.jpg',
      },
      {
        public_id: "test",
       url: 'images/img22.jpg'
      },
    ],
    shop: {
      name: "Cozy Oversized Cuddle Chair",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    price: 1099,
    discount_price: 1049,
    rating: 4,
    total_sell: 75,
    stock: 10,
    category:"Computers & Laptop"
  },
  {
    id: 4,
    category:"Others",
    name: "Elegant Lavender Tufted Accent Chair with Gold Metallic Legs",
    description:
      "Tufted Upholstery:- The backrests of the chairs feature a tufted design, creating a plush, cushioned appearance, which is both elegant and comfortable. Lavender Color:- The chairs are upholstered in a soft lavender or light purple fabric, which gives them a sophisticated and feminine touch, suitable for high-end interiors like lounges, boutique cafes, or luxury salons. Curved Backrest:- The backrests are curved, offering ergonomic support while enhancing the chair's aesthetic appeal. Metallic Legs:- The chairs stand on sleek metallic legs (possibly brass or gold-tone), complementing the elegance of the tufted upholstery while adding a contemporary element.",
    image_Url: [
      {
        public_id: "test",
       url: 'images/img25.jpg'
      },
      {
        public_id: "test",
      url: 'images/img25.jpg'
      },
    ],
    shop: {
      name: "Tufted Accent Chair",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    category:"Others"
    },
    price: 100,
    discount_price: 79,
    rating: 4,
    total_sell: 12,
    stock: 10,
  },
  {
    id: 5,
    category:"Shoes",
    name: "Elegant Lavender Café Setting with Gold Accents and Floral Decor",
    description:
      "Table:- A small round table with a white surface and a golden ornate base complements the overall design. It adds a vintage yet modern touch, likely intended for intimate settings like a café or lounge area. Floral Decor:- To the right of the chairs, there is a large bouquet of pink flowers in a decorative pot, adding a splash of vibrant color. The flowers, along with the greenery in the background, bring a fresh, organic feel to the space. Flooring:- The floor has a marble-like appearance, with glossy white tiles and subtle patterns, contributing to the luxurious feel of the room.Backdrop:- The background features elegant architectural details like columns and molding, evoking a classical, upscale atmosphere. The soft lighting and pastel tones enhance the overall sense of calm and opulence.",
    image_Url: [
      {
        public_id: "test",
       url: 'images/img11.jpg'
      },
      {
        public_id: "test",
       url: 'images/img11.jpg'
      },
    ],
    shop: {
      name: "Elegant Lavender Café",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    price: 120,
    discount_price: 89,
    rating: 5,
    total_sell: 49,
    stock: 10,
    category:"Shoes"
  },
  {
    id: 6,
    name: "blue color sofa",
    description:
      "Product details are a crucial part of any eCommerce website or online marketplace. These details help the potential customers to make an informed decision about the product they are interested in buying. A well-written product description can also be a powerful marketing tool that can help to increase sales.Product details typically include information about the product's features, specifications, dimensions, weight, materials, and other relevant information that can help customers to understand the product better. The product details section should also include high-quality images and videos of the product, as well as customer reviews and ratings.",
    image_Url: [
      {
        public_id: "test",
        url: 'images/51.jpg'
      },
      {
        public_id: "test",
        url: 'images/51.jpg'
      },
    ],
    shop: {
      name: "Asus Ltd",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    price: 300,
    discount_price: 239,
    rating: 4.5,
    reviews: [
      {
        user: {
          // user object will be here
        },
        comment: "IT's so cool!",
        rating: 5,
      },
    ],
    total_sell: 20,
    stock: 10,
    category:"Music and Gaming"
  },
  {
    id: 7,
    name: "modern gray armchair",
    description:
      "Chair Style:- The chair has a modern  minimalist design with clean lines. It features a curved backrest with three buttons tufted into the upholstery. Material :- The chair is upholstered in a soft looking gray or blue-gray fabric. The wooden legs are angled and slim, likely made of polished wood, complementing the overall design . Setting :-  The chair is placed on a wooden floor in what appears to be a living room or reading nook There is a bookshelf in the background with some decor and books ",
    image_Url: [
      {
        public_id: "test",
        url: 'images/img21.jpg'
      },
      {
        public_id: "test",
        url: 'images/img21.jpg'
      },
    ],
    shop: {
      name: "Armchair",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    price: 100,
    discount_price: 79,
    rating: 4,
    total_sell: 62,
    stock: 10,
  },
  {
    id: 8,
    name: "crucial",
    description:
      "Product details are a crucial part of any eCommerce website or online marketplace. These details help the potential customers to make an informed decision about the product they are interested in buying. A well-written product description can also be a powerful marketing tool that can help to increase sales.Product details typically include information about the product's features, specifications, dimensions, weight, materials, and other relevant information that can help customers to understand the product better. The product details section should also include high-quality images and videos of the product, as well as customer reviews and ratings.",
    image_Url: [
      {
        public_id: "test",
        url: 'images/52.png'
      },
      {
        public_id: "test",
         url: 'images/52.png'
      },
    ],
    shop: {
      name: "Asus Ltd",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    price: 300,
    discount_price: 239,
    rating: 4.5,
    reviews: [
      {
        user: {
          // user object will be here
        },
        comment: "IT's so cool!",
        rating: 5,
      },
    ],
    total_sell: 20,
    stock: 10,
  },
  {
    id: 9,
    category:"Mobile and Tablets",
    name: "Iphone 14 pro max 256 gb ssd and 8 gb ram silver colour",
    description:
      "Product details are a crucial part of any eCommerce website or online marketplace. These details help the potential customers to make an informed decision about the product they are interested in buying. A well-written product description can also be a powerful marketing tool that can help to increase sales.Product details typically include information about the product's features, specifications, dimensions, weight, materials, and other relevant information that can help customers to understand the product better. The product details section should also include high-quality images and videos of the product, as well as customer reviews and ratings.",
    image_Url: [
      {
        public_id: "test",
        url: 'images/img22.jpg'
      },
      {
        public_id: "test",
         url: 'images/img22.jpg'
      },
    ],
    shop: {
      name: "Amazon Ltd",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    discount_price: 1099,
    rating: 5,
    total_sell: 20,
    stock: 10,
  },
  {
    id: 10,
    category:"Music and Gaming",
    name: "Gaming Headphone Asus with mutiple color and free delivery",
    description:
      "Product details are a crucial part of any eCommerce website or online marketplace. These details help the potential customers to make an informed decision about the product they are interested in buying. A well-written product description can also be a powerful marketing tool that can help to increase sales.Product details typically include information about the product's features, specifications, dimensions, weight, materials, and other relevant information that can help customers to understand the product better. The product details section should also include high-quality images and videos of the product, as well as customer reviews and ratings.",
    image_Url: [
      {
        public_id: "test",
        url: 'images/img23.jpg'
      },
      {
        public_id: "test",
        url: 'images/img23.jpg'
      },
    ],
    shop: {
      name: "Asus Ltd",
      shop_avatar: {
        public_id: "test",
        url: "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png",
      },
      ratings: 4.2,
    },
    price: 300,
    discount_price: 239,
    rating: 4.5,
    reviews: [
      {
        user: {
          // user object will be here
        },
        comment: "IT's so cool!",
        rating: 5,
      },
    ],
    total_sell: 20,
    stock: 10,
  },
];