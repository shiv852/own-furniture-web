...
import { googleAuth } from "../common/api";
...
const LoginUp = () => {
  ...
  const {fetchUserDatails} =  useContext(Context)
  ...
  console.log("data saved login", data);

  const handleSubmit = async(e) => {
    
    e.preventDefault();
    const dataResponse = await fetch(SummaryApi.signIN.url,{
      method : SummaryApi.signIN.method,     // isme agr cookie m data save krwana h toh credentials include krna pdega abhi browser pr cookie m data save hoga or backend m cors ko check kr lena ek bar
      credentials:'include',
      headers:{
        "content-type" : "application/json"
      },
      body: JSON.stringify(data)     // email password wala data 
    })
    const dataApi = await dataResponse.json()
    if(dataApi.success){
      toast.success(dataApi.message)
      navigate('/')       //redirect to the home page
      fetchUserDatails()  //when user successfully login call this function
                ...
                <div className="max-w-md mx-auto">
                  <form onSubmit={handleSubmit}>
                    <div className="mt-4">
                      ...
                      <div className="flex relative">
                        <input
                          ...
                          className="border rounded-lg px-3 py-2 mt-1 mb-5 w-full h-full"
                          id="password"
                          name="password"
                          onChange={handleOnchange}
                          value={data.password}
                        />
                        <div
                          className="absolute top-4 text-xl right-6 cursor-pointer"
                    ...
                    <div className="text-right mb-2">
                      <Link
                        to="/forgotpassword"
                        className="text-xs no-underline font-display font-semibold text-gray-500 hover:text-gray-600 cursor-pointer hover:underline"
                      >
                        Forgot Password?
                      </Link>
                    </div>
                    <div className="flex justify-center w-full items-center">
                      <div>
                        <button type="button" className="flex items-center justify-center py-2 px-20 bg-white hover:bg-gray-200 focus:ring-blue-500 focus:ring-offset-blue-200 text-gray-700 w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg" onClick={googleLogin}>
                          <svg
                            viewBox="0 0 24 24"
                            height="25"
                            width="25"
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <Link
                      to="/signup"
                      className="text-xs no-underline hover:text-gray-600 uppercase dark:text-gray-400 hover:underline "
                    >
                      or sign up
                    </Link>
